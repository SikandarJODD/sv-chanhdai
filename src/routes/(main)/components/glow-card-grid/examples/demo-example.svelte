<script lang="ts">
	import { GlowCardGrid } from "$lib/components/chan/glow-card-grid";

	const DemoComponent: any = GlowCardGrid;
</script>

<section class="flex w-full flex-col gap-4 rounded-2xl border border-border/60 bg-background p-6 shadow-sm">
	<div class="space-y-1">
		<h2 class="text-lg font-semibold">Glow Card Grid demo</h2>
		<p class="text-sm text-muted-foreground">
			Swap this placeholder with a real usage example for Glow Card Grid.
		</p>
	</div>

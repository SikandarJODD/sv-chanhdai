<script lang="ts">
	import { IconSwap } from "$lib/components/chan/icon-swap";

	const DemoComponent: any = IconSwap;
</script>

<section class="flex w-full flex-col gap-4 rounded-2xl border border-border/60 bg-background p-6 shadow-sm">
	<div class="space-y-1">
		<h2 class="text-lg font-semibold">Icon Swap demo</h2>
		<p class="text-sm text-muted-foreground">
			Swap this placeholder with a real usage example for Icon Swap.
		</p>
	</div>

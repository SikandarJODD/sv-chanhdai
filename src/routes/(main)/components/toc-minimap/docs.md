# Toc Minimap

TODO: Add a concise description for Toc Minimap.

## Installation

<Tabs items={["CLI", "Manual"]}>
<Tab value="CLI">

### Using CLI

```bash
npx shadcn-svelte@latest add https://sv-animations.vercel.app/r/toc-minimap.json
```

</Tab>
<Tab value="Manual">

### Manual Installation

Copy the component source files into your project and update the example files with real usage.

</Tab>
</Tabs>

## Usage

```svelte
<script lang="ts">
  import { TocMinimap } from "$lib/components/chan/toc-minimap";
</script>

<TocMinimap />
```

## Props

Document the Toc Minimap props here.

## Features

- Replace this placeholder bullet with a real Toc Minimap feature.
- Add one or two implementation details that matter to consumers.
- Include usage constraints or accessibility notes if they apply.

# Elastic Slider

TODO: Add a concise description for Elastic Slider.

## Installation

<Tabs items={["CLI", "Manual"]}>
<Tab value="CLI">

### Using CLI

```bash
npx shadcn-svelte@latest add https://sv-animations.vercel.app/r/elastic-slider.json
```

</Tab>
<Tab value="Manual">

### Manual Installation

Copy the component source files into your project and update the example files with real usage.

</Tab>
</Tabs>

## Usage

```svelte
<script lang="ts">
  import { ElasticSlider } from "$lib/components/chan/elastic-slider";
</script>

<ElasticSlider />
```

## Props

Document the Elastic Slider props here.

## Features

- Replace this placeholder bullet with a real Elastic Slider feature.
- Add one or two implementation details that matter to consumers.
- Include usage constraints or accessibility notes if they apply.
